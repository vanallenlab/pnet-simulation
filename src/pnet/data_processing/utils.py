...
def get_sample_cols_from_VCF(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    logging.debug(
        "restricting to just the genotype columns (the ones that end in .GT)..."
    )
...
def convert_binary_var_mat_to_gene_level_mat(
    binary_genotypes, variant_metadata, binary_output=False
):
    """
    ...
    logger.info(
        "2. Group the filtered variant metadata df by gene, get gene-level counts"
    )
    ...
    for gene in set(variant_metadata.SYMBOL.tolist()):
        ...
        logger.debug(
            f"binary_genotypes.loc[curr_vars,:]: {binary_genotypes.loc[curr_vars, :].shape}"
        )
    ...
    if binary_output:
        logger.info(
            "4. Binarizing the gene burden matrix (anything !=0. gets set to 1)"
        )
        gene_burden_mat = gene_burden_mat.applymap(binarize)
...
def filter_VCF_chunk(df, gene_list):
    assert "SYMBOL" in df.columns.tolist(), (
        f"SYMBOL isn't in the columns, which are \n{df.columns.tolist()}"
    )
...
def filter_annotated_vcf_by_gene_list_chunking(
    ...
    list_of_dfs = []
    with pd.read_csv(
        annot_vcf_f, chunksize=chunksize, sep="\t", low_memory=False
    ) as reader:
        for i, chunk in tqdm(enumerate(reader), desc="Filtering VCF by gene list"):
            ...
            if not filtered_df.empty:
                logging.debug(
                    f"In this chunk, filtered down to {len(filtered_df)} rows."
                )
...
def n_variants_per_sample_from_vcf(
    ...
    """
    logging.info(
        f"working with a VCF with {vcf.shape[1]} samples and {vcf.shape[0]} rows (variants, genes, etc)"
    )
