...
class GenesetNetwork:
    def __init__(self, gene_list, path, num_layers=3, sparsity=0.9, trim=0):
        ...
        self.layer_nodes = (
            [self.genes2pathways["pathway"].nunique()]
            + [l for l in layer_node_list if l < self.genes2pathways["pathway"].nunique()][:num_layers]
            + [1]
        )
    ...
    def get_layers(self, sparsity):
        gene_layers = []
        first_gene_layer = (
            pd.get_dummies(self.genes2pathways["gene"]).join(self.genes2pathways["pathway"]).groupby("pathway").sum().T
        )
        gene_layers.append(first_gene_layer)
        for i in range(len(self.layer_nodes) - 2):
            rand_gene_layer = np.random.choice(
                [0, 1], size=(len(self.gene_list), self.layer_nodes[i + 1]), p=[sparsity, 1 - sparsity]
            )
        ...
        pathway_layers = []
        first_pathway_layer = np.random.choice(
            [0, 1], size=(self.layer_nodes[0], self.layer_nodes[1]), p=[sparsity, 1 - sparsity]
        )
        ...
        for i in range(1, len(self.layer_nodes) - 1):
            rand_pathway_layer = np.random.choice(
                [0, 1], size=(self.layer_nodes[i], self.layer_nodes[i + 1]), p=[sparsity, 1 - sparsity]
            )
