...
from pnet import ReactomeNetworkOG, pnet_loader, util
...
class PNET_NN(pl.LightningModule):
    ...
    def __init__(
        ...
        self.input_layer = nn.Sequential(
            *masked_activation(pathway_masks[0], activation=self.activation, batchnorm=True, dropout=self.input_dropout)
        )
        ...
        for i in range(1, len(pathway_masks)):
            self.layers.append(
                nn.Sequential(
                    *masked_activation(
                        pathway_masks[i], activation=self.activation, batchnorm=True, dropout=self.dropout
                    )
                )
            )
            self.preds.append(
...
def fit(model, dataloader, optimizer):
    ...
    for batch in dataloader:
        ...
        if model.loss_weight is not None:
            ...
            aux_losses = [
                (model.loss_fn(y_h, y) * weight_).mean() * w for y_h, w in zip(y_hats, model.aux_loss_weights)
            ]
